...
numFeatures = 121;    % number of features to learn,如果A是n×k，则k=numFeatures，n代表每个样本的维度
...
weightMatrix = randn(visibleSize, numFeatures) * 0.005;% A
featureMatrix = randn(numFeatures, numPatches) * 0.005;% S
...
% 例如第一个feature只在第一个group中，第二个feature只在第二个group中,说明此时不是拓扑结构
