...
clear all
close all
%首先训练rbm网络，rbm中自带了参数w和b的更新
%然后构建decoder和encoder，其中decoder和encoder使用相同的参数w和b
%使Input=Output，交叉熵作为代价函数，用backpropgation和共轭线性搜索来更新整个网络的w和b
...
makebatches;%batchdata=100×784×600，表示有600个batch，每个batch含有100个样本
...
batchdata=batchposhidprobs;%100*1000*600，整个数据正向传播时隐含层的输出概率
numhid=numpen;
restart=1;
rbm;
