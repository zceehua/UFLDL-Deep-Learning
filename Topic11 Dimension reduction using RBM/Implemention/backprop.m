...
%分别装载每层的权值和偏置值，将它们作为一个整体,decoder和encoder使用相同的权值
w1=[vishid; hidrecbiases];%[785,1000]
...
l1=size(w1,1)-1;%每个网络层中节点的个数,784
l2=size(w2,1)-1;
l3=size(w3,1)-1;
l4=size(w4,1)-1;
l5=size(w5,1)-1;
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%共轭梯度线性搜索
  ...
  [X, fX] = minimize(VV,'CG_MNIST',max_iter,Dim,data);%每层网络对应节点的个数（不包括偏置值）
