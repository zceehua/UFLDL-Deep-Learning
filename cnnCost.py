...
delta_s = Wd'*delta_d; %the pooling/sample layer's preactivation，注意这里算出来的是mean pooling层的误差！
...
%对所有输入图像的每个pooling层做unsampling
for i=1:numImages
    for j=1:numFilters
		%进行unsampling操作,由于是mean sampling，所以用ones矩阵
...
for i=1:numFilters
    ...
    for j=1:numImages  
		%算第一层的权值误差是需要将输入图片与每一个特征图相乘，算出每个特征图对应的误差
		%然后权值的误差就是每个相乘后结果的和
