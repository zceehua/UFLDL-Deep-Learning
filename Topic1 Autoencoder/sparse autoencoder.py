...
class Autoencoder(object):
    def __init__(self, n_in, n_hidden, sparsity_level = 0.05, sparse_reg = 0.001):
        ...
        self.sparsity_level= np.repeat([0.05], self.n_hidden).astype(np.float64)
    ...
    def LoadParam(self, weights_file):
        ...
        a=self.hiddenLayer.W.assign(tf.transpose(params['w_hid']))
        b=self.hiddenLayer.b.assign(tf.reshape(params['b_hid'],[200,1]))
        c=self.hiddenLayer.b_prime.assign(tf.reshape(params['b_vis'],[300,1]))
        ...
        for epoch in xrange(n_epoches):
            ...
            for batch in xrange(0, batch_cnt + 1):
                ...
                for param in self.params:
                    ...
                    paraGrad=self.batch_grad[param].assign(np.zeros_like(sess.run(self.batch_grad[param]), dtype=np.float64))
                    sess.run(paraGrad)
            end_time = time.time()
